...
class SalesTestCase(unittest.TestCase):
    ...
    def test_create_sale_record(self):
        ...
        response = self.client().post('/api/v2/auth/signup', headers = {"Authorization":"Bearer " + access_token_admin}, data=json.dumps(self.reg_user), content_type='application/json')
        response = self.client().post('/api/v2/auth/login', data=json.dumps(self.login_user), content_type='application/json')
        json_data = json.loads(response.data)
        access_token_user = json_data.get('access_token')
        response = self.client().post('/api/v2/products', headers = {"Authorization":"Bearer " + access_token_admin}, data=json.dumps(self.product), content_type='application/json')
    ...
    def test_create_sale_as_admin(self):
        ...
        self.assertEqual(response.status_code, 201)
        self.assertIn('Product Successfully Created', str(response.data))
        response = self.client().post('api/v2/sales', headers = {"Authorization":"Bearer " + access_token_admin}, data=json.dumps(self.sale), content_type='application/json')
    ...
    def test_create_sale_record_if_values_are_missing(self):
        ...
        self.assertEqual(response.status_code, 201)
        self.assertIn('Product Successfully Created', str(response.data))
        response = self.client().post('api/v2/sales', headers = {"Authorization":"Bearer " + access_token_user}, data=json.dumps(self.sale2), content_type='application/json')
