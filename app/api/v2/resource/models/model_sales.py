...
class Sales:
    ...
    def add_sale(self, name, quantity):
        ...
        if quantity > get_quantity:
            return {'error':'Quantity order is more than that in stock'}, 401
        ...
        try:
            ...
            connection.commit()
            response = jsonify({'success':'Sale Successfully Created'})
    ...
    def get_all_sales(self):
        ...
        try:
            ...
            sales_list = []
            if sales is None:
                return {'error':'No sales found'}, 200
            if sales:
                for sale in sales:
                    sales_dict = {
                        "sales_id" : sale[0],
    ...
    def get_one_sale(self, saleId):
        ...
        try:
            ...
            if sale is None:
                return {'error':'No sale found'}, 404
            if sale:
                sale_dict = {
                    "sales_id" : sale[0],
                    "product_id" : sale[1],
                ...
                return {'error':'Sorry, this sale was not generated by you'}, 403
        except (Exception, psycopg2.DatabaseError) as error:
            print(error)
            response = jsonify({'error':'Problem fetching record from the database'})
            response.status_code = 400
